...
public class UserAuthValidationFilter : AbstractValidator<UserAuthDto>
{
    public UserAuthValidationFilter()
    {
        RuleFor(c => c.Login)
            ...
            .NotEmpty().WithMessage("Пожалуйста, укажите никнейм.")
            .Length(3, 50).WithMessage("Еикнейм должен быть от 3 до 50 символов.");
            // .Matches("^[a-zA-Z0-9]*$").WithMessage("Никнейм должен состоять только из букв и цифр.");
